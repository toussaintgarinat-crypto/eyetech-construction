import { BrowserRouter, Routes, Route, Navigate } from 'react-router-dom'
import LoginPage from './pages/LoginPage'
import Dashboard from './pages/Dashboard'
import ProjetsPage from './pages/ProjetsPage'
import ProjetDetailPage from './pages/ProjetDetailPage'
import CalqueDetailPage from './pages/CalqueDetailPage'
import TutorielPage from './pages/TutorielPage'
import AidePage from './pages/AidePage'
import NavBar from './components/NavBar'

function PrivateLayout({ children }) {
  const token = localStorage.getItem('access_token')
  if (!token) return <Navigate to="/login" replace />
  return (
    <div style={{ display: 'flex', width: '100%', minHeight: '100vh' }}>
      <NavBar />
      <main style={{ flex: 1, marginLeft: '240px', padding: '32px', background: '#0f172a', minHeight: '100vh' }}>
        {children}
      </main>
    </div>
  )
}

export default function App() {
  return (
    <BrowserRouter>
      <Routes>
        <Route path="/login" element={<LoginPage />} />
        <Route path="/" element={<PrivateLayout><Dashboard /></PrivateLayout>} />
        <Route path="/projets" element={<PrivateLayout><ProjetsPage /></PrivateLayout>} />
        <Route path="/projets/:id" element={<PrivateLayout><ProjetDetailPage /></PrivateLayout>} />
        <Route path="/calques/:id" element={<PrivateLayout><CalqueDetailPage /></PrivateLayout>} />
        <Route path="/tutoriel" element={<PrivateLayout><TutorielPage /></PrivateLayout>} />
        <Route path="/aide" element={<PrivateLayout><AidePage /></PrivateLayout>} />
        <Route path="*" element={<Navigate to="/" replace />} />
      </Routes>
    </BrowserRouter>
  )
}
